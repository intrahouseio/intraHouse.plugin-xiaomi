...
class Xiaomi extends EventEmitter {
  constructor(options) {
    ...
    this.socket_main = dgram.createSocket('udp4');
    this.socket_ext = dgram.createSocket('udp4');
  ...
  setDeviceInitData(sid, data) {
    ...
    if (sid === this.gatewaySettigs.sid) {
      device.data = Object.assign({}, temp1, this.gatewaySettigs, temp2);
    } else {
  ...
  checkDeviceAutoReset(device) {
    if (device.type === 'motion' && device.data.status === 'motion') {
      if (this.timerList[device.sid]) {
        clearTimeout(this.timerList[device.sid]);
      }
      this.timerList[device.sid] = setTimeout(() => {
        this.deviceList[device.sid].data.status = 'no_motion';
        this.emit('data', this.deviceList[device.sid])
      }, 5000);
    }
  }
